...
function goodVsEvil(good, evil){
  let goodMap = [1, 2, 3, 3, 4, 10];
  let evilMap = [1, 2, 2, 2, 3, 5, 10];
  good = good.split(' ').map(num => parseInt(num));
  evil = evil.split(' ').map(num => parseInt(num));
  let goodCount = 0;
  let evilCount = 0;
  for (let i = 0; i < goodMap.length; i++) {
    goodCount += goodMap[i] * good[i];
  }
  for (let i = 0; i < evilMap.length; i++) {
    evilCount += evilMap[i] * evil[i];
  }
